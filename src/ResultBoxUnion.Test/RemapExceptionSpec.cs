...
public class RemapExceptionSpec
{
    ...
    public void RemapExceptionTest1()
    {
        var result = ResultBox.Error(new InvalidDataException("test"))
            .RemapException(ex => ex is ArgumentNullException
                ? new ApplicationException("remapped", innerException: ex)
                : ex);
    ...
    {
        var result = ResultBox.Error(new InvalidDataException("test"))
            .RemapException(ex => ex is InvalidDataException
                ? new ApplicationException("remapped", innerException: ex)
                : ex);
        Assert.False(result.IsSuccess);
        Assert.IsType<ApplicationException>(result.GetException());
        Assert.Equal("remapped", result.GetException().Message);
    }
